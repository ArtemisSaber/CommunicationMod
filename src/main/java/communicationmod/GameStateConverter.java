...
public class GameStateConverter {
  ...
  /**
   * Creates a JSON representation of the status of CommunicationMod that will be
   * sent to the external process.
   * The JSON object returned contains:
   * - "available_commands" (list): A list of commands (strings) available to the
   * user
   * - "ready_for_command" (boolean): Denotes whether the game state is stable and
   * ready to receive a command
   * - "in_game" (boolean): True if in the main menu, False if the player is in
   * the dungeon
   * - "game_state" (object): Present if in_game=True, contains the game state
   * object returned by getGameState()
   * 
   * @return A string containing the JSON representation of CommunicationMod's
   *         status
   */
  ...
  /**
   * Creates a JSON representation of the game state, which will be sent to the
   * client.
   * Always present:
   * - "screen_name" (string): The name of the Enum representing the current
   * screen (defined by Mega Crit)
   * - "is_screen_up" (boolean): The game's isScreenUp variable
   * - "screen_type" (string): The type of screen (or decision) that the user if
   * facing (defined by Communication Mod)
   * - "screen_state" (object): The state of the current state, see
   * getScreenState() (as defined by Communication Mod)
   * - "room_phase" (string): The phase of the current room (COMBAT, EVENT, etc.)
   * - "action_phase" (string): The phase of the action manager
   * (WAITING_FOR_USER_INPUT, EXECUTING_ACTIONS)
   * - "room_type" (string): The name of the class of the current room (ShopRoom,
   * TreasureRoom, MonsterRoom, etc.)
   ...
   * - "potions" (list): A list of the player's potions (empty slots are
   * PotionSlots)
   * - "map" (list): The current dungeon map
   * - "keys" (object): Contains booleans for each of the three keys to reach Act
   * 4
   * Sometimes present:
   * - "current_action" (list): The class name of the action in the action manager
   * queue, if not empty
   * - "combat_state" (list): The state of the combat (draw pile, monsters, etc.)
   * - "choice_list" (list): If the command is available, the possible choices for
   * the choose command
   * 
   ...
   * These extra formatting characters are turned into things like colored or
   * wiggly text in game, but
  ...
  private static String resolveCardDescription(AbstractCard card) {
    String text = card.rawDescription;
    text = text.replace("!D!", String.valueOf(card.isDamageModified ? card.damage : card.baseDamage));
    text = text.replace("!B!", String.valueOf(card.isBlockModified ? card.block : card.baseBlock));
    text = text.replace("!M!", String.valueOf(card.isMagicNumberModified ? card.magicNumber : card.baseMagicNumber));
    for (DynamicVariable dv : BaseMod.cardDynamicVariableMap.values()) {
      String placeholder = "!" + dv.key() + "!";
      if (text.contains(placeholder)) {
        text = text.replace(placeholder, String.valueOf(dv.isModified(card) ? dv.value(card) : dv.baseValue(card)));
      }
    }
   ...
   * "body_text" (string): The current body text for the event, or an empty string
   * if there is none
   * "event_name" (string): The name of the event, in the current language
   * "event_id" (string): The ID of the event (NOTE: This implementation is
   * sketchy and may not play nice with mods)
   * "options" (list): A list of options, in the order they are presented in game.
   * Each option contains:
   * - "text" (string): The full text associated with the option (Eg. "[Banana]
   * Heal 10 hp")
   * - "disabled" (boolean): Whether the current option or button is disabled.
   * Disabled buttons cannot be chosen
   * - "label" (string): The simple label of a button or option (Eg. "Banana")
   * - "choice_index" (int): The index of the option for the choose command, if
   * applicable
   * 
  ...
  private static HashMap<String, Object> getEventState() {
    ...
    if (eventDialogType == ChoiceScreenUtils.EventDialogType.IMAGE
        || eventDialogType == ChoiceScreenUtils.EventDialogType.ROOM) {
    ...
    if (event instanceof NeowEvent) {
      ...
      try {
        // AbstractEvent does not have a static "ID" field, but all of the events in the
        // base game do.
   ...
   * - "reward_type" (string): The name of the RewardItem.RewardType enum for the
   * reward
   ...
   * - "link" (object): The relic that the sapphire key is linked to, if
   * applicable
   * 
   ...
   * "current_node" (object): The node object for the currently selected node, if
   * applicable
   * "next_nodes" (list): A list of nodes that can be chosen next
   * "first_node_chosen" (boolean): Whether the first node in the act has already
   * been chosen
   ...
   * _for_purge" (boolean): Whether the selected cards will be removed from the
   * deck
   * "confirm_up" (boolean): Whether the confirm screen is up, and cards cannot be
   * selected
   * 
   ...
   * "hand" (list): The list of cards currently in your hand, not including
   * selected cards
  ...
  private static HashMap<String, Object> getHandSelectState() {
    ...
    // As far as I can tell, this comment is a Java 8 analogue of a Python list
    // comprehension? I think just looping is more readable.
    // handJson =
    // handCards.stream().map(GameStateConverter::convertCardToJson).collect(Collectors.toCollection(ArrayList::new));
   ...
   * "limbo" (list): The list of cards that are in 'limbo', which is used for a
   * variety of effects in game.
   * "card_in_play" (object, optional): The card that is currently in play, if
   * any.
   * "player" (object): The state of the player
   * "monsters" (list): A list of the enemies in the combat, including dead
   * enemies
   ...
   * "times_damaged" (int): The number of times the player has been damaged this
   * combat (for Blood for Blood).
   * Note: The order of the draw pile is not currently randomized when sent to the
   * client.
   * 
   ...
   * "children" (list, optional): The nodes connected by an edge out of the
   * provided node
   ...
   * "is_playable" (boolean): Whether the card can currently be played, though
   * does not guarantee a target
  ...
  private static HashMap<String, Object> convertCardToJson(AbstractCard card) {
    ...
    jsonCard.put("has_target",
        card.target == AbstractCard.CardTarget.SELF_AND_ENEMY || card.target == AbstractCard.CardTarget.ENEMY);
    jsonCard.put("exhausts", card.exhaust);
    jsonCard.put("ethereal", card.isEthereal);
    jsonCard.put("description", resolveCardDescription(card));
    return jsonCard;
  }
   ...
   * "intent" (string): The name of the AbstractMonster.Intent enum for the
   * monster's current intent
   ...
   * "move_adjusted_damage" (int): The damage number actually shown on the intent
   * for the monster's current attack
  ...
  private static HashMap<String, Object> convertMonsterToJson(AbstractMonster monster) {
    ...
    if (AbstractDungeon.player.hasRelic(RunicDome.ID)) {
      ...
      if (moveInfo != null) {
        ...
        // If isMultiDamage is not set, the multiplier is probably 0, but there is
        // really 1 attack.
   ...
   * Note: many other things, like draw pile and discard pile, are in the combat
   * state
   * 
  ...
  /**
   * Checks whether the given object has the specified field. If so, returns the
   * field's value. Else returns null.
   * 
   ...
   * "card" (object, optional): The card associated with the power (for powers
   * like Nightmare)
   * "misc" (int, optional): Contains misc values that don't fit elsewhere (such
   * as the base value for Flight)
   * "just_applied" (boolean, optional): Used with many powers to prevent them
  ...
  private static ArrayList<Object> convertCreaturePowersToJson(AbstractCreature creature) {
    ...
    for (AbstractPower power : creature.powers) {
      ...
      // justApplied is used with a variety of powers to prevent them from expiring
      // immediately (cast from bool)
   ...
   * "description" (string): The description of the relic, in the currently
   * "used_up" (boolean): If the relic has been exhausted
   * selected language
   * 
  ...
  private static HashMap<String, Object> convertRelicToJson(AbstractRelic relic) {
    ...
    jsonRelic.put("description", stripFormattingCodes(relic.description));
    jsonRelic.put("used_up", relic.usedUp);
    return jsonRelic;
  }
  ...
  private static HashMap<String, Object> convertPotionToJson(AbstractPotion potion) {
    ...
    jsonPotion.put("description", stripFormattingCodes(potion.description));
    return jsonPotion;
  }
